...
RUN mkdir -p /opt/mrh-admin
# Note: Ensure you have an 'admin' folder with your web files
COPY admin /opt/mrh-admin

CMD ["/bin/sh", "-lc", "/usr/local/bin/start-xray.sh && /usr/local/bin/start-admin.sh && tail -f /tmp/xray.log"]
